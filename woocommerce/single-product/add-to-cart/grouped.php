...
<form class="cart grouped_form" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data'>
	<table class="group_table w-100p">
		<tbody>
			<?php
			...
			foreach ($grouped_products as $grouped_product_child) {
				...
				foreach ($grouped_product_columns as $column_id) {
					...
					$class_list = 'woocommerce-grouped-product-list-item__' . esc_attr($column_id);
					...
					switch ($column_id) {
						case 'quantity':
							...
							if (!$grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || !$grouped_product_child->is_in_stock()) {
								...
								do_action('woocommerce_after_add_to_cart_quantity', true);
						...
						case 'price':
							...
							$value = $grouped_product_child->is_on_sale() ? making_stuffs_sale_price_html() : $grouped_product_child->get_price_html();
							break;
						default:
							$value = '';
							break;
					...
					echo '<td class="' .  $class_list . '">' . apply_filters('woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child) . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	<?php if ($quantities_required) : ?>
		<div class="stuffs-row wrap justify-even">
